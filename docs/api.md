...
첫 `/workspace` 요청에서 HttpOnly 쿠키로 세션을 발급합니다. 요청자가 임의의 작업 공간 ID를 지정할 수 없으며, 아래 데이터는 모두 서버가 세션에서 확인한 작업 공간에 속합니다. JSON 요청 본문은 최대 64,000바이트로 제한하고, 다른 출처에서 보내는 변경 요청은 거절합니다.
