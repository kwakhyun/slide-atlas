...
현재는 템플릿의 최신 버전만 보관하고, 프레젠테이션의 `templateVersion`과 비교해 변경을 감지합니다. 과거 버전의 배치 정보를 변경 불가능한 사본으로 저장하는 기능은 아직 구현하지 않았습니다. 사용 중인 템플릿이 수정되면 품질 경고를 표시하며, 사용자가 템플릿을 다시 선택해 원문을 재배치하도록 안내합니다. 따라서 버전 정보만으로 과거 레이아웃을 완전히 복원할 수는 없습니다.
