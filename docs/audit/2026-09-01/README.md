# Slide Atlas 기능·UI/UX 감사 보고서

- 감사 일자: 2026-09-01
- 대상: [공개 서비스](https://slide-atlas-mu.vercel.app)와 현재 소스 코드
- 화면 크기: 데스크톱 1280×900, 모바일 390×844
- 기준 사용자: 처음 방문한 채용 담당자·면접관, 프레젠테이션 템플릿을 등록·생성·검수·평가하는 운영자
- 확인 범위: 스튜디오, 라이브러리, 검수 인박스, 실험실, 프로젝트 가이드, 발표 화면, 내보내기 메뉴, 모바일 반응형, 콘솔 오류, 자동화 테스트와 API 경계

> **문서 읽는 방법:** 아래 `개선 전 사용자 여정별 상태`부터 `개선 전 구현 순서 제안`까지는 최초 감사 시점의 기준선과 발견 사항을 보존한 기록입니다. 현재 상태와 완료 판정은 `개선 반영 결과` 이후에서 확인하실 수 있습니다.

## 판단

현재 프로젝트는 **채용 포트폴리오로 제출 가능한 수준**입니다. 브리프 입력부터 구조 검색, 슬롯 기반 편집, 품질 검사, 검수, 검색 실험, PPTX 내보내기까지 하나의 제품 흐름으로 연결되어 있으며, 화면 완성도와 기술적 근거도 충분합니다. 치명적인 사용성 결함이나 공개 화면의 콘솔 오류는 발견하지 못했습니다.

가장 큰 개선 기회는 화면을 더 꾸미는 일이 아니라 **운영자가 다음 작업으로 자연스럽게 이동하도록 연결하고, 변경·검수·실험의 근거를 비교 가능한 형태로 보여주는 일**입니다. 현재도 각 기능은 잘 작동하지만 라이브러리→스튜디오, 검수→변경점 확인, 실패 질의→다음 실험의 연결 동작이 약합니다.
...
- 프레젠테이션 메뉴에 이름 변경·복제·삭제를, 필름스트립에 드래그 순서 변경·삭제·다시 실행을 추가합니다.
...
- AI 생성 결과에 모델·프롬프트 버전·토큰·소요 시간·남은 일일 요청 수를 접을 수 있는 근거 패널로 표시합니다.
...
- 검색 결과에 `관련도순·최근 수정순·상태순` 정렬을 추가하고, 점수 툴팁에서 키워드·의도·구조·용량 기여도를 설명합니다.
...
- 템플릿 버전 스냅샷을 저장하고 슬롯·용량·레이아웃·샘플 문구의 변경점을 나란히 비교합니다.
...
| -------- | ---------------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------ |
| P1       | 라이브러리→스튜디오, 실패 질의→라이브러리 연결 | 기능 묶음이 하나의 운영 워크플로우로 보입니다  | 템플릿 상세와 실험 하단에서 선택 문맥을 유지한 채 다음 화면으로 이동합니다     |
| P1       | 검수 버전 스냅샷과 변경점 비교                 | 검수 정확도와 제품의 실무 설득력이 커집니다    | 이전·현재 버전의 슬롯, 용량, 레이아웃, 문구 차이를 확인하고 승인할 수 있습니다 |
| P1       | 프레젠테이션·슬라이드 관리와 충돌 복구         | 반복 사용 가능한 내부 도구가 됩니다            | 이름 변경·복제·삭제, 슬라이드 순서 변경·삭제·다시 실행, 409 복구가 가능합니다  |
| P1       | 품질 표현을 근거 중심으로 변경                 | 과도한 신뢰를 줄이고 검수 기준을 명확히 합니다 | 점수 대신 통과·오류·경고 수와 각 검사 근거를 우선 표시합니다                   |
| P2       | 모바일 입력·미리보기 분할 모드                 | 작은 화면의 편집 왕복 비용이 줄어듭니다        | 390px에서 한 번의 탭으로 입력과 확대 미리보기를 전환합니다                     |
| P2       | AI 진행 단계·생성 근거·남은 예산 표시          | 대기 불안과 운영 비용 불확실성이 줄어듭니다    | 생성 단계와 모델·프롬프트·토큰·시간·남은 요청 수를 확인할 수 있습니다          |
| P2       | 실행 간 비교와 실패 사례 필터                  | 실험 결과가 다음 개선으로 이어집니다           | 두 실행 비교, 실패·개선·하락 필터, 평가셋·카탈로그 해시를 제공합니다           |
| P3       | 첫 방문 3분 데모 체크리스트와 문구 정리        | 평가자가 프로젝트 가치를 더 빨리 이해합니다    | 첫 방문 안내와 한국어 우선 레이블을 적용하고 다시 열 수 있습니다               |
...
위에서 제안한 P1·P2·P3 항목은 모두 구현했습니다. 라이브러리에서 Studio로 이어지는 템플릿 적용, 템플릿 버전 스냅샷과 변경점 비교, 프레젠테이션·슬라이드 관리, 충돌 복구, 근거 중심 품질 표시, 모바일 입력·미리보기 전환, AI 진행·실행 근거, 실험 실행 비교·필터·CSV·후속 탐색, 첫 방문 3분 안내를 제품 흐름에 반영했습니다.

후속 검토에서 PPTX 텍스트·좌표·글자 크기를 온톨로지 후보로 바꾸는 제한된 추출 PoC를 추가했습니다. 콘텐츠·온톨로지·편집·검수·실험 담당자의 완료 기준을 역할 기반 Playwright 시나리오 5개로 고정해 모두 통과했으며, 이 과정에서 발견한 이름 저장 버튼 회귀도 수정했습니다. Library 상세·등록·가져오기, Studio 대화상자와 전역 스타일은 기능 단위 파일로 분리했습니다.
...
- 구현 커밋 `e906878`의 소스와 테스트를 대조했습니다. 저장소에는 생성·저장·PPTX 생성·추출·발표, 템플릿 등록→검수→승인, 실험 실행, API 충돌, 데스크톱·모바일 접근성 시나리오가 포함되어 있습니다.
