...
| 항목           | 내용                                                                                                  |
| -------------- | ----------------------------------------------------------------------------------------------------- |
| 형태           | 미리디 AI 프레젠테이션 프로덕트 엔지니어 포지션을 참고해 기획한 개인 프로젝트                         |
| 개발·검증 기간 | 2026.08.31–2026.09.01                                                                                 |
| 개인 기여 범위 | 문제 정의, 데이터 모델, UI/UX, 프론트엔드, REST API, DB, AI 연동, 테스트, 배포                        |
| 구현 데이터    | 직접 제작한 템플릿 18개, 전달 의도 6종, 슬롯·제약·검수 상태                                           |
| 핵심 검색 결과 | 합성 개발 질의 24개에서 Hit@1 **70.8% → 91.7% (+20.8%p)**, MRR **0.7431 → 0.9167**                    |
| 검증·운영 환경 | Vitest **57개**, Playwright **24개**, 운영 역할 시나리오 **5/5**, PostgreSQL CI, Neon, OpenAI, Vercel |

검색 수치는 직접 만든 개발용 데이터로 측정했으며 일반적인 검색 성능이나 실제 운영 성과를 의미하지 않습니다.
...
[프로젝트 기획](docs/product-brief.md) · [아키텍처](docs/architecture.md) · [REST API](docs/api.md) · [운영 역할 검증](docs/operator-validation.md) · [배포·검증 기록](docs/verification.md) · [평가 원본](docs/evaluation.json) · [보안·한계](SECURITY.md)
...
템플릿을 `전달 의도 → 레이아웃 → 슬롯 → 제약 조건`으로 정의했습니다. 텍스트나 수치를 담는 영역인 슬롯에는 역할, 정규화된 좌표, 글꼴 크기, 글자 수 제한, 필수 여부를 지정했습니다. 화면에서 등록할 때, JSON을 가져올 때와 PPTX에서 추출한 초안을 검토할 때 동일한 Zod 스키마로 검증하며, 중복 키·범위를 벗어난 좌표·필수 제목 누락·잘못된 예시 데이터를 차단합니다.

PPTX PoC는 ZIP 중앙 디렉터리를 먼저 검사한 뒤 최대 12장의 OOXML에서 텍스트 상자와 배치 좌표를 읽습니다. 반복 구조·숫자·시점 표현·좌우 배치를 근거로 레이아웃과 슬롯 역할을 추론하고, 지원하지 않는 이미지·표·차트는 경고로 남깁니다. 추출 요청만으로 데이터가 저장되지는 않습니다.
...
| 의사결정      | 선택                                       | 이유와 감수한 한계                                                                                                   |
| ------------- | ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| 디자인 데이터 | 상태·버전은 관계형 컬럼, 슬롯 구조는 JSONB | 트랜잭션과 유연한 스키마를 함께 사용했습니다. 대규모 검색에는 DB 후보 축소와 페이지네이션이 추가로 필요합니다.       |
...
로컬 PGlite와 **PostgreSQL 17을 사용하는 GitHub CI에서 전체 검증 절차를 실행**합니다. [CI 워크플로우](https://github.com/kwakhyun/slide-atlas/actions/workflows/ci.yml)에서 의존성 설치, 정적 검사, 57개 테스트, 평가 재현, 프로덕션 빌드와 24개 브라우저 시나리오를 확인하실 수 있습니다. 재현 명령과 배포 중 발견한 문제 및 해결 과정은 [배포·검증 기록](docs/verification.md)에 정리했습니다.
...
- 역할 기반 브라우저 시나리오 5개를 통과했지만 외부 참여자를 대상으로 한 관찰 연구는 아닙니다. 수치·단위·주장 사이의 관계 검증, 실제 글꼴 렌더링 측정, 독립된 평가 데이터와 운영자 3~5명의 사용성 관찰을 다음 검증 대상으로 정했습니다.
