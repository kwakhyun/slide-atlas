...
| 로컬 검증      | Vitest **84개**, Playwright **31개** 통과, 타입 검사와 프로덕션 빌드 통과      |
...
위 화면은 2026-09-05 로컬 프로덕션 빌드의 규칙 기반 모드입니다.
...
| Vitest                                 | 로컬 **84개 통과**: 스키마, 검색, 품질, PPTX, DB 트랜잭션, 격리, 버전 사본, 덱 관리, AI 출력 검증                  |
| Playwright                             | 로컬 **31개 통과**; 이전 PostgreSQL CI **25개**, 2026-09-01 공개 배포 **24개 통과**: 핵심 흐름, PPTX, 충돌, 모바일 |
...
- 템플릿의 과거 버전은 변경 불가능한 사본으로 보관하고 최근 20개를 비교할 수 있습니다. 기존 덱은 생성 당시 사본으로 표시하고 저장하며, 새 승인 버전은 편집 내용을 보존해 직접 적용할 수 있습니다. 참조 무결성은 저장소 계층에서 검증하며 과거 버전 복원과 일괄 갱신은 후속 과제입니다.
