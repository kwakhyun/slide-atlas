...
**[라이브 데모 열기](https://slide-atlas-mu.vercel.app)** | [제품 흐름 영상](docs/demo/slide-atlas-walkthrough.webm) | [역할 요건과 구현 근거](docs/portfolio-readiness.md)
...
| 개발·검증 기간 | 2026.08.31–2026.09.05                                                          |
...
| 로컬 검증      | Vitest **107개**, Playwright **52개** 통과, 타입 검사와 프로덕션 빌드 통과     |

로그인이나 API 키 없이 **LLM을 호출하지 않는 규칙 기반 모드**를 체험하실 수 있습니다. 작업 결과는 PPTX 또는 JSON으로 내려받을 수 있습니다. 공개 서비스는 전용 Neon PostgreSQL과 초대 코드 방식의 OpenAI 생성을 연결했으며, 2026-09-04까지 확인한 운영 상태와 실제 AI 호출 결과는 [배포·검증 기록](docs/verification.md)에 정리했습니다.
...
위 화면과 제품 흐름 영상은 2026-09-05 로컬 프로덕션 빌드의 규칙 기반 모드입니다. [화면별 스크린샷과 재촬영 방법](docs/screenshots/README.md)에서 촬영 조건을 확인할 수 있습니다.
...
2026-09-05 로컬 수정본에서는 수치의 부호와 단위 보존, 사용자 정의 슬롯 매핑, 생성 당시 템플릿 버전 참조와 저장 중 편집 잠금을 추가했습니다. 생성이 끝나면 내용 편집 탭에서 확인이 필요한 슬라이드를 바로 볼 수 있습니다. [수정 내용과 검증 범위](docs/improvements-2026-09-05.md)를 정리했으며, 이번 변경의 공개 배포는 아직 확인하지 않았습니다.
...
- OpenAI 생성 중에는 대기 안내를 표시하고, 생성 후에는 모델, 프롬프트 버전, 토큰, 소요 시간과 남은 일일 요청 수를 확인하실 수 있습니다.
...
추가 검토에서는 저장 응답을 바로 반영하도록 바꾸고, 연속 입력의 되돌리기와 미저장 상태 판정을 개선했습니다. 모바일 메뉴에 이름을 표시하고 안내 글자 크기를 키웠으며, 검수 이력과 실험 결과는 해당 화면에서 조회합니다. 편집 이력, 저장 처리, 입력과 미리보기 모듈을 분리하고 변경하지 않은 슬라이드의 검사 결과와 SVG 미리보기를 재사용합니다. 구현과 측정 범위는 [디자인·구조·최적화 개선 기록](docs/editor-improvements-2026-09-05.md)을 참고해 주세요.

## UI/UX 리메이크

밝은 작업 공간과 청록색 강조를 중심으로 전체 화면을 다시 구성했습니다. 편집기에는 좁은 탐색 메뉴와 넓은 캔버스를, 템플릿 화면에는 큰 검색창과 아이콘 카테고리를 적용했습니다. 검수, 실험, 계정과 공유 화면도 같은 디자인 규칙을 사용합니다. 모바일과 태블릿의 편집 흐름까지 확인했으며, [변경 내용](docs/design-remake-2026-09-05.md)과 [디자인 QA](design-qa.md)에 범위를 정리했습니다.

## 기능 고도화

Studio에서 브라우저 초안 복구, 경고 입력란 이동, 실제 SVG 넘침 검사, 슬롯별 재생성과 원문 근거 연결을 사용할 수 있습니다. 브랜드 스타일은 버전으로 보관하며, 라이브러리에서는 템플릿 변경 영향 확인과 선택 적용, PPTX 후보 일괄 등록을 지원합니다.

실험실에서는 평가셋과 가중치 설정 사본을 저장해 비교합니다. `계정 / 팀`에서는 자체 계정, 초대와 역할을 관리하고, Studio에서는 검수 댓글과 만료되는 읽기 전용 공유를 제공합니다. 사용 방법, 권한표와 한계는 [기능 고도화 기록](docs/feature-upgrades-2026-09-05.md)에 정리했습니다.

## 팀 운영과 품질 검증 확장

작성자의 검수 요청과 검수자의 승인 권한을 분리하고, 변경 계정과 버전을 감사 이력에 연결했습니다. 일괄 등록은 서버에서 중복을 방지하며, 변경 영향은 전후 슬라이드와 직접 수정한 내용으로 검토할 수 있습니다. PPTX 후보 교정, 고정된 결과의 두 평가자 검토, 의미 검색 비교와 서버 작업 복구도 제공합니다. [사용 방법과 검증 경계](docs/operational-upgrades-2026-09-05.md)를 참고하세요.
...
키워드 일치도에 온톨로지 동의어, 전달 의도, 슬롯 수, 수용 가능한 내용량을 함께 반영합니다. PostgreSQL에서 작업 공간과 필터, 텍스트 후보를 먼저 줄이고 구조 점수를 계산한 뒤 페이지 단위로 반환합니다. 검색 결과에는 점수의 근거를 표시하고, 일치하는 항목이 없으면 빈 결과를 반환합니다. 평가용 정답은 검색기에 전달하지 않습니다. 이 기준선은 임베딩 없이 구조 정보를 추가한 효과를 확인합니다. 실험실에는 실제 임베딩과 구조 점수의 조합을 비교하는 별도 기능을 추가했으며, 해당 기능의 실제 검색 품질은 아직 측정하지 않았습니다.
...
| 범위                                   | 검증 결과 · 로컬 수정본과 기존 배포 기록 구분                                                                      |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| TypeScript / ESLint / production build | 통과                                                                                                               |
| Vitest                                 | 로컬 **107개 통과**: 스키마, 검색, 품질, PPTX, DB 트랜잭션, 격리, 버전 사본, 덱 관리, AI 출력 검증                 |
| Playwright                             | 로컬 **52개 통과**; 이전 PostgreSQL CI **25개**, 2026-09-01 공개 배포 **24개 통과**: 핵심 흐름, PPTX, 충돌, 모바일 |
| 역할 기반 자동화 시나리오              | Playwright로 콘텐츠, 온톨로지, 편집, 검수, 실험 과제 **5/5 통과**; 외부 사용자 검증은 실행 대기                    |
| 접근성                                 | 6개 화면 × 1440px/390px, 대화상자에 axe WCAG A/AA 자동 위반 없음; 키보드 순환 확인                                 |
...
2026-09-05 로컬 수정본의 [검증 기록](docs/local-verification.json)은 기존 공개 배포와 구분합니다. 로컬 PGlite와 **PostgreSQL 17을 사용하는 GitHub CI에서 전체 검증 절차를 실행**하도록 구성했습니다. 이전 [2026-09-04 검증 기록](docs/verification.md)에서는 59개 단위·통합 테스트, 25개 브라우저 시나리오, 검색 평가, 홀드아웃 카탈로그, AI 입력 평가, PPTX 평가와 프로덕션 빌드 결과를 확인할 수 있습니다. 재현 명령과 배포 중 발견한 문제는 [배포·검증 기록](docs/verification.md)에 정리했습니다.
